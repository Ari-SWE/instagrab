...
export class WebsiteInstagramDataProvider implements InstagramDataProvider {
  ...
  private async getBrowser(): Promise<Browser> {
    if (!this.browser || !this.browser.connected) {
      ...
      logger.info('Puppeteer browser launched with cloud container flags');
    }
    return this.browser;
  }
  ...
  async getAllMedia(username: string): Promise<InstagramMediaResult> {
    ...
    try {
      logger.info(`Fetching data for @${username} via multi-provider scraper`);

      let capturedUserInfo: any = null;
      let capturedPosts: InstagramPost[] = [];
      let capturedStories: InstagramStory[] = [];
      let capturedHighlights: InstagramHighlight[] = [];
        ...
        if (res.status() === 200) {
          try {
            if (u.includes('/api/v1/instagram/userInfo')) {
              ...
              logger.info(`[storiesig] Captured userInfo for @${username}`);
            } else if (u.includes('/api/v1/instagram/postsV2')) {
              const json = await res.json();
              const edges = json.result?.edges || [];
              capturedPosts = edges.map((e: any, idx: number) => {
                const node = e.node || e;
              ...
              logger.info(`[storiesig] Captured ${capturedPosts.length} posts for @${username}`);
            } else if (u.includes('/api/v1/instagram/stories')) {
              const json = await res.json();
              const list = json.result || json.data || [];
              capturedStories = list.map((s: any, idx: number) => {
                ...
                const bestCandidate = candidates.length > 0
                  ? candidates.reduce((best: any, c: any) => ((c.width || 0) > (best.width || 0) ? c : best), candidates[0])
                  : null;
              ...
              logger.info(`[storiesig] Captured ${capturedStories.length} stories for @${username}`);
            } else if (u.includes('/api/v1/instagram/highlights')) {
              const json = await res.json();
              const list = json.result || json.data || [];
              capturedHighlights = list.map((h: any, idx: number) => {
                const rawCover =
                ...
                const parsedItems: InstagramStory[] = rawItems.length > 0
              ...
              logger.info(`[storiesig] Captured ${capturedHighlights.length} highlights for @${username}`);
            }
          } catch {}
        }
      });
      ...
      if (capturedUserInfo) {
        ...
        if (profile.isPrivate) {
          throw Errors.privateAccount();
        }

        logger.info(
          `Successfully extracted @${username}: ${capturedPosts.length} posts, ${capturedStories.length} stories, ${capturedHighlights.length} highlights`,
        );
      ...
      logger.info(`Falling back to insta-stories-viewer.com for @${username}`);
      ...
      `);
      if (notFoundVisible) throw Errors.userNotFound();
      ...
      `);
      if (isPrivate) throw Errors.privateAccount();
      ...
      let finalHighlights: InstagramHighlight[] = capturedHighlights;
      ...
      logger.error(`Failed to scrape Instagram media: ${err.message}`);
